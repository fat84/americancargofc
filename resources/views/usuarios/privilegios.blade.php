...
@section('content')
    ...
    @if(isset($user))
        <div class="box-body row">
            <h5 class="text-capitalize"><strong>Privilegios Del Usuario:</strong> {{$user->persona->nombre}} {{$user->persona->apellido}}</h5>
        </div>
    @endif

    @if(isset($allprivilegios) && isset($userprivilegios) && isset($user))
        <div class="box">

            {{ Form::open(['route' => ['updateprivilegios', $user->id], 'method' => 'POST']) }}
                ...
                <table class="table table-hover">
                    ...
                    @foreach($allprivilegios as $privilegio)
                        ...
                        <?php $crear=0; $editar=0; $eliminar=0; $consultar=0;
                            foreach($userprivilegios as $userpri) {
                                if($userpri->privilegio_id == $privilegio->id){
                                    $crear = $userpri->crear;
                                    $editar = $userpri->editar;
                                    $eliminar = $userpri->eliminar;
                                    $consultar = $userpri->consultar;
                                }
                            }
                        ?>
                        <tr>
